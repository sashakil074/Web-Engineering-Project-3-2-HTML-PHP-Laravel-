...
<div class="container" style="background-color:white;margin: auto;width: 50%;border-style:outset;" >
                ...
                @if (count($errors) > 0)
                    ...
                    <div class="publisher bt-1 border-light"> <img class="avatar avatar-xs" src="{{asset('images')}}/{{$picdata2[0]->ProfilePic}}"
                     alt="..."><form action="{{route('send.Message')}}" method="post" enctype="multipart/form-data" >
                     @csrf
                     <input type="hidden" name="PsyUsername"  value="{{$picdata[0]->Username}}" />
                      <input class="publisher-input" type="text" 
                    placeholder="Write something" name="PtMessage">  
